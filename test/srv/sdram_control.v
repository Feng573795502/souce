...
`include        "Sdram_Params.h"
   ...
   //SDRAM命令信号组合
	...
	begin
		if(!Rst_n)
			...
			ref_time_cnt <= 1;
		else if(init_done || ref_time_cnt > 0)    //初始化完成/第二次运行就进入自动刷新计数
	...
	//刷新定时时间标志位，定时到达时置1
	assign ref_time_flag = (ref_time_cnt == AUTO_REF); //自动刷新标志
			...
			case(main_state)
				...
				WRITE:begin
					if(FF == 1'b0)          //写操作
						...
						if(ref_req == 1'b1)begin            //跳转到自动刷新
							main_state <= AREF;
							FF <= 1'b0;
						end
						else if(wr_opt_done & wr_req)begin //写完成&有写请求进入写操作
							main_state <= WRITE;
							FF <= 1'b0;
						end
						else if(wr_opt_done & rd_req)begin //写完成&读请求
							main_state <= READ;
							FF <= 1'b0;
						end
						else if(wr_opt_done&!wr_req&!rd_req)  //写操作完成 并且没有新的读写请求 回到之刷新
							main_state <= AREF;
						else
							main_state <= WRITE;
					end
   ...
   localparam 
	...
	begin
		if(!Rst_n)
			ref_cnt <= 16'd0;
		else if(ref_cnt == ref_END)      //结束跳回0
			ref_cnt <= 16'd0;
		else if(ref_req || ref_cnt>1'b0) //等待自动刷新请求
	...
	begin
		if(!Rst_n)
			ref_opt <= 1'b0;
		else if(ref_req == 1'b1)		//有写请求的话标志位写入
			ref_opt <= 1'b1;            
		else if(ref_opt_done == 1'b1)   //完成清空
			ref_opt <= 1'b0;
		else
			ref_opt <= ref_opt;
	end
